...
public class FrontendServlet extends HttpServlet {
            ...
            throws IOException
    {
        ...
        // Check if the file is actually a reports directory
        String reportsPath = requestedFile;
        String lastCharacter = String.valueOf(reportsPath.charAt(reportsPath.length() - 1));
        if(File.separator.equals(lastCharacter)){
            reportsPath = reportsPath.substring(0, reportsPath.length()-1);
        }

        // URL-decode the file name (might contain spaces and on) and prepare file object.
        File file = new File(basePath, URLDecoder.decode(requestedFile, "UTF-8"));

        File reportsDirectory = file;
        if(StringUtils.containsIgnoreCase(reportsPath, "/report/") && !reportsDirectory.isFile()){
            ...
            if(reportsDirectory.isDirectory()){
                ...
                for (File reportFile: reportsDirectory.listFiles()) {
                    ...
                    filePath = reportFile.getPath().replaceAll("\\\\+", "/");
                    filePath = filePath.replaceAll("/+", "/");
                    fileInfo.addProperty("path", filePath.substring(basePath.length()-1));
                    fileInfo.addProperty("name", reportFile.getName());
                    fileInfo.addProperty("date", reportFile.lastModified());
                    fileInfo.addProperty("size", reportFile.length());
                ...
                try {
                    ...
                    output.write(directoryList.toString().getBytes());
                } finally {
                    output.close();
                }
            }
